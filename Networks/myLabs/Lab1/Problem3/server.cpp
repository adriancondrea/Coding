...
int main() {
    ...
    while (1) {
        ...
        int res = recv(c, (char*)&sizeof_r, sizeof(sizeof_r), 0);
        if (res != sizeof(sizeof_r)) 
            printf("Error receiving sizeof_r\n");
        sizeof_r = ntohs(sizeof_r);
        res = recv(c, (char*)&a, sizeof_r + 1, 0);
        //check we got the string
        if (res != sizeof_r  +1) 
            printf("Error receiving string\n");
        //reinitializing the size of the reversed string
        sizeof_r = 0;
