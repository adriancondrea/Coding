...
int main() {
    ...
    while (1) {
        ...
        uint16_t arraySize, arrayElement, sum = 0;
        ...
        int res = recv(c, (char*)&arraySize, sizeof(arraySize), 0);
        //check we got an unsigned short value
        if (res != sizeof(arraySize)) 
            printf("Error receiving array size!\n");
        arraySize = ntohs(arraySize);
        for(uint16_t i = 0; i < arraySize; i++)
        {
            res = recv(c, (char*)&arrayElement, sizeof(arrayElement), 0);
            if (res != sizeof(arrayElement)) 
                printf("Error receiving array element!\n");
            arrayElement = ntohs(arrayElement);
            sum += arrayElement;
