...
int main() {
    ...
    while (1) {
        ...
        uint16_t number, d, divisors[100], nrDiv = 0;
        ...
        int res = recv(c, (char*)&number, sizeof(number), 0);
        ...
        number = ntohs(number);
        for(d = 1; d * d < number; d++){
            if(number % d == 0){
                divisors[nrDiv++] = d;
                divisors[nrDiv++] = number/d;
            }
        }
        if(d * d == number)
            divisors[nrDiv++] = d;
        ...
        nrDiv = htons(nrDiv);
        res = send(c, (char*)&nrDiv, sizeof(nrDiv), 0);
            if (res != sizeof(nrDiv)) 
                printf("Error sending number of divisors!\n");
        //convert it back to  host byte
        nrDiv = ntohs(nrDiv);
        for(uint16_t i = 0; i < nrDiv; i++){
            divisors[i] = htons(divisors[i]);
            res = send(c, (char*)&divisors[i], sizeof(divisors[i]), 0);
            if (res != sizeof(divisors[i])) 
                printf("Error sending divisor!\n");
        }
